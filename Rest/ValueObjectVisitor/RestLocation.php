...
class RestLocation extends ValueObjectVisitor
{
    ...
    public function visit( Visitor $visitor, Generator $generator, $data )
    {
        ...
        $generator->startValueElement( 'mainLanguageCode', $data->location->contentInfo->mainLanguageCode );
        ...
        $generator->startValueElement( 'name', $data->location->contentInfo->name );
        $generator->endValueElement( 'name' );

        $generator->startValueElement(
            'canEdit',
            $this->serializeBool($generator, $data->canEdit)
        );
        $generator->endValueElement('canEdit');

        $generator->startValueElement(
            'canCreate',
            $this->serializeBool($generator, $data->canCreate)
        );
        $generator->endValueElement('canCreate');

        $generator->startValueElement(
            'canMove',
            $this->serializeBool($generator, $data->canMove)
        );
        $generator->endValueElement('canMove');

        $generator->startValueElement(
            'canRemove',
            $this->serializeBool($generator, $data->canRemove)
        );
        $generator->endValueElement('canRemove');
